<template>
  <div class="container mt-4 order-details" v-if="order" ref="orderDetails">
    ...
    <div class="order-info">
      ...
      <div><strong>Inicio de reserva:</strong> {{ formatDate(order.reserves[0].start_reserve) }}</div>
      <div><strong>Final de reserva:</strong> {{ formatDate(order.reserves[0].finish_reserve) }}</div>
      <div><strong>Estado:</strong> <span :class="getStatusClass(order.status)">{{ getStatusText(order.status) }}</span>
      </div>
    </div>
    ...
    <button v-if="order.status === 'pending'" @click="showCancelConfirmationDialog" :disabled="pushingCancel"
      class="btn btn-danger btn-cancel m-2">
      Cancelar pedido
